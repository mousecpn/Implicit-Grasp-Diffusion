...
class VGN_RVIZ(object):
    def __init__(self, model_path, rviz=False, best=True, force_detection=True, qual_th=0.5):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.net = load_network(model_path, self.device, 'vgn')
        self.rviz = rviz
        self.best = best
        self.force_detection = force_detection
        self.qual_th = qual_th
    ...
    def __call__(self, state):
        ...
        qual_vol = bound(qual_vol, voxel_size)
        grasps, scores = select(qual_vol.copy(), rot_vol, width_vol, force_detection=self.force_detection, threshold=self.qual_th)
        toc = time.time() - tic
