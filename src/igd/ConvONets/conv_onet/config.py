...
import os
from igd.ConvONets.encoder import encoder_dict
from igd.ConvONets.conv_onet import models, training
from igd.ConvONets.conv_onet import generation
from igd.ConvONets import data
from igd.ConvONets import config
from igd.ConvONets.common import decide_total_volume_range, update_reso
...
def get_model(cfg, device=None, dataset=None, **kwargs):
    ...
    if tsdf_only:
        ...
        if diff:
            decoder_rot = models.decoder_dict["diff_local"](
            c_dim=c_dim, padding=padding, out_dim=7,
            **decoder_kwargs
        )
        else:
