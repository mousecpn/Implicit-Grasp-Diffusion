...
from igd.dataset_voxel import DatasetVoxel
from igd.networks import get_network, load_network

LOSS_KEYS = ['loss_all', 'loss_qual', 'loss_rot']
...
def create_train_val_loaders(root, root_raw, batch_size, val_split, augment, kwargs):
    # load the dataset
    dataset = DatasetVoxel(root, root_raw, augment=augment)
...
def prepare_batch(batch, device):
    ...
    return pc, (label, rotations, width), pos
...
def create_trainer(net, optimizer, loss_fn, metrics, device):
    def _update(_, batch):
        ...
        # forward
        x, y, pos = prepare_batch(batch, device)
